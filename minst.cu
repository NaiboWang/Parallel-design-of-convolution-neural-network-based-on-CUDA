...
ImgArr read_Img(const char* filename) // 读入图像
{
	...
	// 图像数组的初始化
	ImgArr imgarr;//imgarr是保存的原始图像的最外层结构体
	cudaMallocManaged(&imgarr, sizeof(MinstImgArr));
	imgarr->ImgNum=number_of_images;//指定数据集大小,60000或10000
	imgarr->c = n_cols;
	imgarr->r = n_rows;
	cudaMallocManaged(&imgarr->ImgPtr, number_of_images*n_cols*n_rows * sizeof(float));
	int tab = n_cols * n_rows;//相隔距离
	for(i = 0; i < number_of_images; ++i)  //分别对图像进行赋值操作
	{
		for(r = 0; r < n_rows; ++r)      
		{
			...
			{ 
				...
				imgarr->ImgPtr[i*tab+r*n_cols+c]=(float)temp/255.0;//归一化成0到1之间的float
			}  
		}    
	}
	fclose(fp);
...
LabelArr read_Lable(const char* filename)// 读入图像
{
	...
	// 图像标记数组的初始化
	LabelArr labarr;
	cudaMallocManaged(&labarr, sizeof(MinstLabelArr));
	labarr->LabelNum=number_of_labels;
	labarr->l = 10;
	cudaMallocManaged(&labarr->LabelPtr, number_of_labels * 10 * sizeof(float));
	for(i = 0; i < number_of_labels; ++i)  
	{  
		for (int j = 0;j < 10;j++)
			labarr->LabelPtr[i * 10 + j] = 0.0;
		unsigned char temp = 0;  
		fread((char*) &temp, sizeof(temp),1,fp);//读取标签值,这个值是0~9的值 
		labarr->LabelPtr[i * 10 + (int)temp]=1.0;    //因此,给LabelData[temp]赋值为1.0表示这个图像的正确标签为temp所指向的那一位
	}
	fclose(fp);
	return labarr;	
}
...
void save_Img(ImgArr imgarr,char* filedir) // 将图像数据保存成文件
{
	...
	for(i=0;i<img_number;i++){
		...
		//	fwrite(imgarr->ImgPtr[i].ImgData[r],sizeof(float),imgarr->ImgPtr[i].c,fp);//把imgarr->ImgPtr[i].ImgData[r]整个数组写入fp中,为什么是整个数组,因为imgarr->ImgPtr[i].ImgData[r]代表数组第一个元素,然后个数指定的是imgarr->ImgPtr[i].c,就是写入了整个数组
